...
if active.empty:
    st.error("No active sprint! Start one in Team & System Setup.", icon=":material/error:")
else:
    s_data = active.iloc[0]
    s_id = s_data['id']
    team_df = get_team()
    ...
    with st.container(border=True):
        ...
        # Calculate capacity details for selected dev
        dev_row = team_df[team_df['name'] == owner].iloc[0]
        s_start_dt = pd.to_datetime(s_data['start_date'])
        s_end_dt = pd.to_datetime(s_data['end_date'])
        wk_days = get_workdays(s_start_dt, s_end_dt)

        hols_in_sprint = len(get_holidays(s_id, s_start_dt.date(), s_end_dt.date()))
        leaves_df = get_leaves(s_id)
        dev_leaves_sum = leaves_df[(leaves_df['name'] == owner) & (leaves_df['sprint_id'] == s_id)]['total_days'].sum()
        ...
        # Live metric cards instead of plaintext caption
        ...
        if st.button("Commit ticket", type="primary"):
            ...
            add_ticket(s_id, tid, title, owner, role, cat, sp)
            st.rerun()
